"""
speed_hex_timeout.py

Strict-timeout speed Hex for the appendix experiment.

This variant removes the no-timeout safety net used by the existing Hex setup:
if a player spends past their remaining clock, they immediately lose.
...
class Hex(core.Env):
    ...
    def _step(self, state: core.State, action_input, key) -> State:  # type: ignore[override]
        ...
        timed_out = new_time[idx] <= 0
        new_time = jnp.maximum(new_time, 0)

        # Apply pure board step (no clocks)
        state = _step_board(state, action)

        # Base termination and rewards from Hex rules, overridden by timeout loss.
        terminated = state.terminated | timed_out
        timeout_rewards = jnp.where(
            idx == 0,
            jnp.float32([-1.0, 1.0]),
            jnp.float32([1.0, -1.0]),
        )
        rewards = jnp.where(timed_out, timeout_rewards, state.rewards)
