...
class Config(BaseModel):
    ...
    save_dir: str = "./checkpoints/clock/hex/base"
...
def read_hyperparameters_from_csv(task_id: int, csv_path: str) -> dict:
    ...
    Returns a dictionary with the hyperparameters or None if not found.
    """
    with open(csv_path, 'r') as file:
        reader = csv.DictReader(file)
        for i, row in enumerate(reader, start=1):  # Start at 1 to match SLURM array IDs
            if i == task_id:
...
# If task_id is provided (from SLURM array job), read from CSV
...
# CLI arguments override CSV values if provided
...
if __name__ == "__main__":
    ...
    wandb.init(project="hex-base", config=config.model_dump(), name=run_name, dir=config.save_dir)

    print("JAX devices:", jax.local_devices())
    print("num_devices:", num_devices)

    # Create master RNG key from config.seed
    ...
    model = forward.init(init_key2, dummy_input)  # (params, state)
    opt_state = optimizer.init(params=model[0])
    # replicates to all devices
    model, opt_state = jax.device_put_replicated((model, opt_state), devices)

    # Prepare checkpoint dir (include num_simulations and seed in folder name)
    ...
    # Split main loop RNG key from master
