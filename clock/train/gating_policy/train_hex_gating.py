#!/usr/bin/env python3
"""
train_gate_adaptive.py

Single-file PPO training script for a **gate policy** (choose MCTS simulation budget per move)
with adaptive opponent mixing and periodic evaluation.

UPDATED (per your request)
1) Starting time budgets sweep updated to:
   [100, 300, 600, 900, 1200, 1800, 2300, 2900, 3500, 4100, 4800, 5700]

2) Evaluation every EVAL_INTERVAL (default 100):
   - For EACH time budget above
   - For EACH opponent in the eval suite
   - Run EVAL_NUM_GAMES (default 200) games per (budget, opponent)
   => produces a full grid of results.

3) From the grid, compute an Elo-like score per tuple (time_budget, opponent):
   - Convert expected score p to Elo-difference via:
        elo = 400 * log10(p/(1-p))   (with p clamped away from 0 and 1)
   - Rank all tuples by this Elo score
   - Select the 15 lowest (worst) tuples
   - Use these 15 tuples as "hard cases" to upsample in training

4) Training mixture (after warmup):
   - 30% self-play
   - 20% uniform over ALL (time_budget x opponent) tuples
   - 50% uniform over bottom-15 tuples (the worst Elo tuples)

IMPORTANT implementation detail for (4):
- The mixture is applied PER-EPISODE at reset time inside the rollout.
  Each environment episode is assigned a (time_budget, opponent) tuple, and that assignment
  persists until the episode ends (then re-sampled).
"""
...
def parse_args():
    ...
    p.add_argument(
        "--env",
        type=str,
        default=os.getenv("SPEED_ENV", "speed_gardner_chess"),
...
GATE_CKPT_ROOT_ENV = os.getenv("GATE_CKPT_ROOT", "./checkpoints/clock/hex/gating")
...
EVAL_FIXED_OPPONENTS = _parse_int_list_envvar("EVAL_FIXED_OPPONENTS", "2,8,32,128", min_len=1)
...
# WandB
WANDB_PROJECT = os.getenv("WANDB_PROJECT", f"gate_adaptive_training-psim{PRETRAINED_NSIM}")
...
def main():
    ...
    run_name = (
        f"tuplemix_{env_module_name}_pre{PRETRAINED_NSIM}_"
