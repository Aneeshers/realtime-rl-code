#!/usr/bin/env python3
"""
Strict-timeout GRU gate training for speed Hex.

This appendix variant keeps the original trainer structure but changes the task:
when the selected search budget overspends the remaining clock, the environment
ends immediately with a timeout loss. There is no blueprint/policy fallback.
Training remains self-play only.
"""
...
TRAIN_P_SELFPLAY = float(os.getenv("TRAIN_P_SELFPLAY", "1.00"))
TRAIN_P_ALLCOMBO = float(os.getenv("TRAIN_P_ALLCOMBO", "0.00"))
TRAIN_P_BOTTOM15 = float(os.getenv("TRAIN_P_BOTTOM15", "0.00"))
# Safety check
...
def build_all_combo_tables() -> Tuple[np.ndarray, np.ndarray, np.ndarray, List[Tuple[int, str]]]:
    ...
    opponent_labels += ["random", "greedy", "midpeak"]
    ...
    def rollout_core(
        ...
        *,
        ...
        def step_fn(carry, _):
            ...
            actions = cand_actions[gate_action, batch_idx]
            time_spent = sim_options_arr[gate_action].astype(jnp.int32)

            # In the strict-timeout setting, every live self-play move is a gate decision.
            gate_mask = jnp.where(
                is_self_env,
                ~(state.terminated | state.truncated),
                (~(state.terminated | state.truncated) & is_p0),
            ).astype(jnp.float32)
    ...
    def play_many(
        ...
        def play_one(rng_key):
            ...
            def step_fn(carry, step_idx):
                ...
                a = jax.lax.cond(
                    my_time > 0,
                    lambda _: mcts_action(),
                    lambda _: pol_action,
                    operand=None,
                )
                time_spent = jax.lax.select(my_time > 0, intended_cost, jnp.int32(0))
