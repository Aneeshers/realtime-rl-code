...
Supports:
  - --env speed_go  OR  other future speed_* envs
...
CKPT_ROOT_DEFAULT = os.getenv(
    "CKPT_ROOT",
    "./checkpoints/clock/go/base",
)
GATE_ROOT_DEFAULT = os.getenv(
    "GATE_ROOT",
    "./checkpoints/clock/go/gating",
)

SIM_OPTIONS_DEFAULT = [16, 32, 64, 96]
...
def _resolve_flat_az_ckpt(root: str, iter_filename: str) -> Optional[str]:
    ...
    (e.g. ``checkpoints/clock/go/base/base_planner.ckpt``) rather than under the
...
def _resolve_flat_gate_ckpt(gate_root: str, gate_iter: Optional[int] = None) -> Optional[str]:
    ...
    ``gate_root`` (e.g. ``checkpoints/clock/go/gating/gate.pkl``)
    rather than under the ``{env}/pre_{nsim}/opts..._seed{seed}/`` training layout."""
...
def main():
    ...
    parser.add_argument("--env", type=str, default="speed_go",
    ...
    parser.add_argument("--pretrained_nsim", type=int, default=16)
    parser.add_argument("--pretrained_seed", type=int, default=0)
    
    parser.add_argument("--times", nargs="+", type=int, required=True)
    parser.add_argument("--seeds", type=str, required=True)

    parser.add_argument("--sim_options", type=str, default="16,32,64,96")
    parser.add_argument("--opponents", type=str, default="0,16,32,64,96,random")
    ...
    parser.add_argument("--wandb_project", type=str, default="gating_policy_eval_go9x9")
