...
Key behavioral rule:
  - Timeout is enforced.
    If a side selects a search budget that exceeds its remaining clock,
    the environment terminates with a timeout loss.
...
def find_gate_ckpt_gru_style(
    ...
    #   1) New "universal" layout:
    #        pre{N}/opts{...}_seed{S}/gate_{iter}.pkl
    #   2) New PFSP layout:
    #        pre{N}/pfsp_opts{...}_seed{S}/gate_{iter}.pkl
    #   3) Old GRU layout (with hidden size in dir name):
    #        pre_{N}/opts{...}_gru{H}_seed{S}/gate_gru_{iter}.pkl
    opts_str = '-'.join(map(str, sim_options))
    candidates = [
        # (1) New training layout (universal gate)
        os.path.join(gate_root, env_name, f"pre{pretrained_nsim}", f"opts{opts_str}_seed{seed}"),
        # (2) PFSP layout (your pfsp_gating_gru runs)
        os.path.join(gate_root, env_name, f"pre{pretrained_nsim}", f"pfsp_opts{opts_str}_seed{seed}"),
        # Old GRU layout is added below after we know the directory exists
    ]
    # Add old-style with wildcard for GRU hidden size
    old_base = os.path.join(gate_root, env_name, f"pre_{pretrained_nsim}")
    if os.path.isdir(old_base):
        old_prefix = f"opts{opts_str}_gru"
        old_suffix = f"_seed{seed}"
        for d in os.listdir(old_base):
            if d.startswith(old_prefix) and d.endswith(old_suffix):
                candidates.append(os.path.join(old_base, d))
    ...
    for base in candidates:
        ...
        if gate_iter is not None:
            for pattern in [
                f"gate_{gate_iter:06d}.pkl",
                f"gate_periodic_{gate_iter:06d}.pkl",
                f"gate_gru_{gate_iter:06d}.pkl",
                f"gate_final_{gate_iter:06d}.pkl",
            ]:
        ...
        # Some trainings save "best", others save "final" (optionally numbered).
        best_names = ["gate_best.pkl", "gate_gru_best.pkl"]
        final_names = ["gate_final.pkl", "gate_gru_final.pkl"]
        if prefer_best:
            for bn in best_names:
                p = os.path.join(base, bn)
                if os.path.exists(p):
                    return p
        # Prefer un-numbered final if present
        for fnm in final_names:
        ...
        for fn in os.listdir(base):
            m = re.match(r"gate_(?:gru_)?(\d+)\.pkl$", fn)
            if m:
                gates.append((int(m.group(1)), os.path.join(base, fn)))
            m_periodic = re.match(r"gate_periodic_(\d+)\.pkl$", fn)
            if m_periodic:
                gates.append((int(m_periodic.group(1)), os.path.join(base, fn)))
            m2 = re.match(r"gate_final_(\d+)\.pkl$", fn)
            if m2:
                gates.append((int(m2.group(1)), os.path.join(base, fn)))
        if gates:
            gates.sort()
            return gates[-1][1]
...
# ================================================================
# Jitted play-many (strict-timeout variant)
#
# Opponent rules:
        ...
        def step_fn(carry, step_idx):
            ...
            def decisions_when_alive(_):
                ...
                def _gate_argmax_choice(_):
                    return jnp.argmax(logits_gate_b[0]).astype(jnp.int32)
                ...
                # ---------- Policy-only rules ----------
                is_p1_turn = (cur == 1)
                p1_policy_only = is_p1_turn & P1_FORCED_POLICY_ONLY

                # Strict-timeout eval never converts an overspend into a free fallback.
                p1_timeout_avoided = jnp.bool_(False)

                # ---------- P0 policy-only rules ----------
                is_p0_turn = (cur == 0)
                p0_policy_only = jnp.bool_(False)
                p0_timeout_avoided = jnp.bool_(False)
...
def run_eval_once_multiclass(
    ...
    print("Timeout is ENFORCED for both sides: overspending the selected search budget loses on time.")
...
def _replay_and_save_hex_svg(
    ...
    for t in move_indices:
        ...
        if nsim_spent == 0:
            choice_desc = f"P{player}: policy-only"
        else:
            if player == 0:
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            ...
            "STRICT-TIMEOUT VARIANT: selected search budget is charged to the clock, "
            "and overspending loses immediately on time.\n"
        )
    )

    parser.add_argument("--env", type=str, default="speed_gardner_chess",
                        help="Speed env module name, e.g. speed_gardner_chess or speed_hex_timeout")
    ...
    parser.add_argument("--output_dir", type=str, default="gate_eval_multiclass_results_timeout")
    ...
    for T in times:
        ...
        for seed in seeds:
            ...
            if args.wandb and args.wandb_mode != "disabled":
                ...
                project = args.wandb_project or f"gate_eval_timeout_{env_bundle.base_env_id}"
                tags = [t.strip() for t in args.wandb_tags.split(",") if t.strip()] or [
                    "eval", "gate", "multiclass", "timeout", env_bundle.base_env_id
                ]
            ...
            for opp_label, opp_kind, opp_nsim in opponent_specs:
                ...
                if run is not None:
                    ...
                    log_dict = {
                        ...
                        f"{opp_label}/total_expected_score": summary.get("total_expected_score", 0.0),
                        # P1 policy-only
