#!/bin/bash
# Train the PPO gating policy on top of a frozen Snake (SnakeKT) AlphaZero planner.

set -euo pipefail
cd "$(dirname "$0")/../../.."

CKPT_ROOT=${CKPT_ROOT:-./checkpoints/committed_action/snake}
AZ_CHECKPOINT_PATH=${AZ_CHECKPOINT_PATH:-${CKPT_ROOT}/base/k3/training_state_best.pkl}
GATING_CHECKPOINT_DIR=${GATING_CHECKPOINT_DIR:-${CKPT_ROOT}/gating}
SEED=${SEED:-1}

PYTHONPATH=committed_action python -m jumanji.training.train_snake_kt_gating_ppo \
  --env_name              SnakeKT-v1                      \
  ...
  --wandb_project         ${WANDB_PROJECT:-snake_kt_gating_ppo} \
  --seed                  ${SEED}
