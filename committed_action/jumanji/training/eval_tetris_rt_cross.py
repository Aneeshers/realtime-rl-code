...
The noop (5) is the natural delay action: gravity advances but the piece
does not move horizontally or rotate.

Note on MCTS planning: _k_step is gated on _is_pacman so MCTS tree expansion
...
Usage:
    python -m jumanji.training.eval_tetris_rt_cross \
        --k_model 2 \
        --k_eval_list 1 2 3 4 \
        --sims_list 32 64 96 \
        --eval_batch_size 100 \
        --wandb_project tetris_rt_cross_eval \
        --seed 42
"""
...
def eval_one_episode_kstep(
    ...
    Args:
        agent:       GumbelAlphaZeroAgent with num_simulations already set.
    ...
    def body_fun(carry):
        ...
        #    Once early_done triggers, freeze state and stop accumulating.
        def do_step(carry, a):
            ...
            # Freeze state at first termination
        ...
        # Timestep after the last of the K steps (used for next observation)
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="Index of the trained checkpoint directory (1, 2, 3, or 4).",
    )
    parser.add_argument(
        "--base_model_dir",
        type=str,
    ...
    agent = GumbelAlphaZeroAgent(
        ...
        pacman_action_delay=args.k_model,  # initialise to training K (ignored by TetrisRT _k_step)
    )
    ...
    for k_eval in args.k_eval_list:
        # K_eval controls eval-loop execution depth only (MCTS tree uses single-step
        # dynamics for TetrisRT regardless, since _k_step is gated on _is_pacman)
