"""
Cross-model evaluation for SnakeKT Gumbel-AlphaZero.
...
SnakeKT uses policy-guided delay steps rather than a no-op:
  - K_eval-1 "delay" steps: argmax(masked_policy_logits) - no MCTS, direct network call
  - 1 final step: action chosen by full MCTS
  - MCTS tree also uses the same policy-guided K_eval-step dynamics

The action mask is respected at each delay step so the snake never moves into a
wall or its own body during the delay window.

Reward metric: RAW (undiscounted) episode return.

Usage:
    python -m jumanji.training.eval_snake_kt_cross \\
        --k_model 1 \\
        --k_eval_list 1 2 3 4 \\
        --sims_list 32 64 96 128 \\
        --eval_batch_size 100 \\
        --wandb_project snake_kt_cross_eval \\
        --seed 42
"""
...
def eval_one_episode_kt(
    ...
    Per agent decision:
      1. Execute K_eval-1 "delay" steps where action = argmax(masked_policy_logits),
         i.e. direct network forward pass, no MCTS tree search.
         The valid-action mask is applied at each delay step.
    ...
    Args:
        ...
        eval_env:     Raw (unwrapped) SnakeKT environment.
    ...
    def body_fun(carry):
        ...
        # 2. Policy-guided delay steps: K_eval-1 steps using masked argmax of logits.
        def delay_step(carry, _):
            s, ts, cum_r, early_done = carry
            obs = ts.observation
            # Snake observation already has obs.grid (H, W, 5) - no special encoding needed
            grid_b = obs.grid[None, ...]                               # (1, H, W, 5)
            time_b = agent._time_features(jnp.array([s.step_count]))  # (1, 2)
            ...
            # Apply Snake action mask before argmax
            invalid = ~obs.action_mask  # (4,) - True where action is invalid
            masked_logits = jnp.where(~invalid, logits[0], jnp.finfo(logits.dtype).min)
            a = jnp.argmax(masked_logits).astype(jnp.int32)
...
def main():
    parser = argparse.ArgumentParser(
        description="Cross-model SnakeKT evaluation: load K_model checkpoint, "
                    "evaluate over K_eval x num_sims grid. "
                    "Delay steps use argmax(masked_policy_logits) instead of no-op."
    )
    parser.add_argument(
        "--k_model",
        type=int,
        required=True,
        help="K used when training the checkpoint (matches snake_kt_models_{k}).",
    )
    parser.add_argument(
        "--base_model_dir",
        type=str,
        default="./checkpoints/committed_action/snake/base/k",
    ...
    parser.add_argument("--wandb_project", type=str, default="snake_kt_cross_eval")
        ...
        for num_sims in args.sims_list:
            ...
            wandb.init(
                ...
                config={
                    ...
                    "delay_step_type": "masked_policy_logits_argmax",
