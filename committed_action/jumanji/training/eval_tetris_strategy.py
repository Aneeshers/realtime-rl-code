"""Strategy-bin evaluation for the TetrisRT adaptive gating policy.
...
Usage:
  python -m jumanji.training.eval_tetris_strategy \\
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--wandb_project", type=str, default="tetris_rt_strategy_eval")
    p.add_argument("--wandb_entity",  type=str, default=None)
    p.add_argument("--no_wandb",      action="store_true")
    p.add_argument("--output_dir",    type=str, default="tetris_strategy_eval_results")
    return p.parse_args()
...
def compute_strategy_bins(
    ...
    mean_freq = bin_k_freq.mean(axis=0)
    se_freq   = bin_k_freq.std(axis=0) / np.sqrt(N)
