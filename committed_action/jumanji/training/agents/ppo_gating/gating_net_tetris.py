"""Gating network for adaptive MCTS depth selection (TetrisRT).
...
only needs to detect board difficulty, not predict full policy/value, so lower
capacity suffices. Also takes frozen AZNet intermediate features as input.

Inputs:
  - obs_grid:        (B, H, W, 2)  — TetrisRT encoded observation (Ch0=locked board, Ch1=falling piece)
...
class GatingNetTetris(hk.Module):
...
class GatingParamsState(NamedTuple):
    ...
    No net_state field: GatingNetTetris uses LayerNorm which has no running
    statistics, so hk.transform (not hk.transform_with_state) is sufficient.
    """
    params: hk.Params
