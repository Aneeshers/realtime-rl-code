"""
Cross-model evaluation for PacManKT Gumbel-AlphaZero.
...
PacManKT uses policy-guided delay steps rather than noops:
...
Unlike plain PacMan (noop=4), the delay steps here respect the action mask at each
intermediate state, so the agent can never move through walls during the delay.

Reward metric: RAW (undiscounted) episode return.

Usage:
    python -m jumanji.training.eval_pacman_kt_cross \\
        --k_model 2 \\
        --k_eval_list 1 2 3 \\
        --sims_list 32 64 96 \\
        --eval_batch_size 100 \\
        --wandb_project pacman_kt_cross_eval \\
        --seed 42
"""
...
def eval_one_episode_kt(
    ...
    Args:
        ...
        eval_env:     Raw (unwrapped) PacManKT environment.
    ...
    def body_fun(carry):
        ...
        def delay_step(carry, _):
            ...
            grid_b = agent._encode_obs_pacman_unbatched(obs)[None, ...]  # (1, 31, 28, 6)
            ...
            # Apply PacMan action mask before argmax
            invalid = ~obs.action_mask  # (5,) — True where action is invalid
...
def main():
    parser = argparse.ArgumentParser(
        description="Cross-model PacManKT evaluation: load K_model checkpoint, "
                    "evaluate over K_eval × num_sims grid. "
                    "Delay steps use argmax(masked_policy_logits) instead of noop."
    )
    parser.add_argument(
        "--k_model",
        type=int,
        required=True,
        help="K used when training the checkpoint (matches pac_man_kt_models_{k}).",
    )
    parser.add_argument(
        "--base_model_dir",
        type=str,
    ...
    parser.add_argument("--wandb_project", type=str, default="pacman_kt_cross_eval")
