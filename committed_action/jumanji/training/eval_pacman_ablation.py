"""Input-ablation feature-importance analysis for the PacMan gating policy.
...
Usage:
  python -m jumanji.training.eval_pacman_ablation \\
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--wandb_project", type=str, default="pacman_ablation_eval")
    p.add_argument("--wandb_entity",  type=str, default=None)
    p.add_argument("--no_wandb",      action="store_true")
    p.add_argument("--output_dir",    type=str, default="pacman_ablation_results")
    return p.parse_args()
...
def main():
    ...
    # ablation_mode is a static arg so JAX compiles one version per condition.
    # The env/MCTS always use real features; only the gating input is ablated.
