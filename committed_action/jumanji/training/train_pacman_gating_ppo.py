"""PPO Gating Policy for Adaptive MCTS Depth (PacMan).
...
The frozen pretrained AZNet (from pac_man_kt_models_1) provides:
  - Policy logits for K-1 delay steps
...
from jumanji.training.agents.ppo_gating import GatingNet, GatingParamsState
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--eval_meta_steps", type=int, default=1200,
                   help="Max meta-steps per eval episode. Must exceed time_limit/K_min to ensure "
                        "K=1 episodes (time_limit=1000) fully terminate. 1200 > 1000/K=1.")
    ...
    p.add_argument(
        ...
        metavar=("S1", "S2", "S3", "S4"),
        help="MCTS simulation counts for K=1,2,3,4 (e.g. --sim_options 4 8 16 32)",
    )
...
def load_az_checkpoint(checkpoint_dir_or_path: str) -> AlphaZeroParamsState:
    ...
    # Move to first device (eval mode, single device)
...
def single_meta_step_collect(
    ...
    Returns:
        (next_states, next_obs, transition_data)
    where transition_data contains everything needed for PPO.
    """
    ...
    # For pytree states and obs: vmap lax.switch over the batch dim.
    # _sel4 is a module-level function to avoid Python closure capture bugs.
...
def compute_smdp_gae(
    ...
    Returns:
        ...
        value_targets (T, B)
    """
    # V(s_{t+1}) for each t: shift values by 1 and bootstrap at the end
    ...
    # Reverse scan: process from t=T-1 down to t=0
...
def ppo_loss_fn(
    ...
    batch values have shape (MB, ...) after flattening T and B dims.
    """
    obs_grid    = batch["obs_grid"]     # (MB, 31, 28, 6)
    ...
    # Recompute logits and values on the full mini-batch with current params
...
def make_ppo_update_step(
    ...
    """Return a JIT-compiled single-step PPO update, closing over static objects."""
    @jax.jit
    def _step(
        gating_params: hk.Params,
...
def ppo_epoch_update(
    ...
    update_step_fn,                # result of make_ppo_update_step
    data: Dict,
    ...
    # Flatten T and B into a single dimension
...
def main():
    ...
    if args.az_checkpoint_path:
        ...
        # Default: best checkpoint from dir
    ...
    # Construct checkpoint dir with reward_mode, sim options, az epoch, and seed as subdirectories
    # so different experiments never overwrite each other.
    ...
    dummy_grid  = jnp.zeros((1, 31, 28, 6))
    ...
    # Build the JIT-compiled PPO step function (closes over static objects)
    ...
    def collect_rollout(gating_params, init_states, init_obs, key):
        step_keys = jax.random.split(key, args.meta_steps)  # (T, 2)
    ...
    def jit_eval(gating_params, init_states, init_obs, key, *, greedy, random_policy, force_k=-1):
        ...
        Reward accumulation always uses raw (undiscounted) env rewards regardless of
        args.reward_mode, so eval metrics are comparable across different training runs.
        """
        az_net_params = jax.lax.stop_gradient(az_params_state.params.net)
        az_net_state  = jax.lax.stop_gradient(az_params_state.net_state)
        ...
        def scan_body(carry, step_key):
            ...
            # Always use reward_mode="raw" in eval so scores are comparable across
            # training runs that may use different reward_mode values.
            results = [
                meta_step_one_k(agents[i], az_net_params, az_net_state,
    ...
    _benv = VmapAutoResetWrapper(jumanji.make("PacManKT-v1"))
    ...
    for epoch in range(args.num_epochs):
        ...
        if epoch % args.eval_every == 0:
            ...
            eval_env = VmapAutoResetWrapper(jumanji.make("PacManKT-v1"))
            es, ets = eval_env.reset(jax.random.split(eval_key, args.eval_num_envs))

            # Gating policy eval (greedy, eval_num_envs episodes)
            ...
            # Always save an epoch checkpoint at eval time
            save_gating_checkpoint(
                gating_state,
