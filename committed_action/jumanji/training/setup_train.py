...
from jumanji.training.training_types import ActingState, TrainingState
...
def setup_logger(cfg: DictConfig) -> Logger:
    ...
    checkpoint_file_name = getattr(cfg.logger, "checkpoint_file_name", "training_state")
    checkpoint_dir = getattr(cfg.logger, "checkpoint_dir", "checkpoints")
    ...
    if cfg.logger.type == "tensorboard":
        ...
        logger = WandbLogger(
            name=cfg.logger.name,
            project=getattr(cfg.logger, "project", "jumanji"),
            cfg=cfg,
            entity=getattr(cfg.logger, "entity", None),
            save_checkpoint=cfg.logger.save_checkpoint,
            checkpoint_file_name=checkpoint_file_name,
            checkpoint_dir=checkpoint_dir,
        )
        ...
        raise ValueError(
            f"logger expected in ['neptune', 'tensorboard', 'wandb','terminal'], got {cfg.logger}."
        )
    return logger


def _make_raw_env(cfg: DictConfig) -> Environment:
    if cfg.env.name == "flat_pack_clock":
        return FlatPackClock(
            sim_cost=int(getattr(cfg.env.az, "sim_cost", 32)),
            time_limit=int(getattr(cfg.env.az, "time_limit", 576)),
        )
...
def setup_agent(cfg: DictConfig, env: Environment) -> Agent:
    ...
    if cfg.agent == "random":
    ...
    elif cfg.agent == "gumbel_alphazero":
        agent = GumbelAlphaZeroAgent(
            ...
            # AZ/MCTS hyperparams
            num_simulations=cfg.env.az.num_simulations,
            gumbel_scale=cfg.env.az.gumbel_scale,
            learning_rate=cfg.env.az.learning_rate,
            gamma=cfg.env.az.discount_factor,
        
            # Network
            num_channels=cfg.env.az.num_channels,
...
def _setup_random_policy(cfg: DictConfig, env: Environment) -> RandomPolicy:
    ...
    elif cfg.env.name in {"snake", "snake_k_t"}:
        assert isinstance(env.unwrapped, Snake)  # SnakeKT subclasses Snake
    ...
    elif cfg.env.name in {"tetris_rt", "tetris_rt_k_step", "tetris_rt_k_t"}:
        assert isinstance(env.unwrapped, TetrisRT)  # TetrisRTKStep and TetrisRTKT are subclasses
        random_policy = networks.make_random_policy_pacman()  # generic discrete(6) policy
    ...
    elif cfg.env.name == "flat_pack_clock":
        assert isinstance(env.unwrapped, FlatPackClock)
        random_policy = networks.make_random_policy_flat_pack(flat_pack=env.unwrapped)
    elif cfg.env.name in {"pac_man", "pac_man_k_t"}:
        assert isinstance(env.unwrapped, PacMan)  # PacManKT is a subclass of PacMan
