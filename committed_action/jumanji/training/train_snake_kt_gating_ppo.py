"""PPO Gating Policy for Adaptive MCTS Depth (SnakeKT).
...
The frozen pretrained AZNet (from snake_kt_models_3) provides:
...
Each meta-step:
  ...
  2. K-1 policy-guided steps (argmax masked_policy_logits) execute in the environment
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--eval_meta_steps", type=int, default=500,
                   help="Max meta-steps per eval episode (500 >> typical Snake episode length).")
    ...
    p.add_argument("--az_checkpoint_dir", type=str,
                   default="./checkpoints/committed_action/snake/base/k3")
    ...
    p.add_argument("--gating_checkpoint_dir", type=str,
                   default="./checkpoints/committed_action/snake/gating")

    # Logging
    p.add_argument("--wandb_project", type=str, default="snake_kt_gating_ppo")
...
def make_agents(env: VmapAutoResetWrapper, num_envs: int,
                sim_options: List[int] = None,
                gamma: float = 0.997) -> List[GumbelAlphaZeroAgent]:
...
def meta_step_one_k(
    ...
    states: Any,
    obs: Any,
    obs_grid: jnp.ndarray,    # (B, H, W, 5) - pre-computed
...
def ppo_loss_fn(
    ...
    obs_grid    = batch["obs_grid"]      # (MB, H, W, 5)
...
def main():
    ...
    # -- Agents (4 K values) --
    agents = make_agents(env, args.num_envs, args.sim_options, gamma=args.gamma)
    ...
    # -- Infer Snake grid dims from env spec --
    _dummy_obs = raw_env.observation_spec.generate_value()
    H, W = _dummy_obs.grid.shape[0], _dummy_obs.grid.shape[1]
    grid_channels = _dummy_obs.grid.shape[2]
    logger.info(f"SnakeKT grid: H={H}, W={W}, channels={grid_channels}, obs_grid shape: (B, {H}, {W}, {grid_channels})")

    # -- Gating network init --
    gating_fwd = make_gating_forward()

    key, init_key = jax.random.split(key)
    dummy_grid  = jnp.zeros((1, H, W, grid_channels))
    ...
    _benv = VmapAutoResetWrapper(jumanji.make(args.env_name))
    ...
    for epoch in range(args.num_epochs):
        ...
        if epoch % args.eval_every == 0:
            ...
            eval_env = VmapAutoResetWrapper(jumanji.make(args.env_name))
