...
register(id="GraphColoring-v1", entry_point="jumanji.environments:GraphColoring")
...
# Clock-augmented FlatPack: 3x3 blocks (9 blocks, 7x7 grid) with per-step clock deduction.
# sim_cost ticks are deducted each step; time_left exposed in observation for clock-aware values.
register(id="FlatPackClock-v0", entry_point="jumanji.environments:FlatPackClock")
...
# TetrisRT - real-time Tetris where each step is one gravity tick (6 discrete actions).
register(id="TetrisRT-v0", entry_point="jumanji.environments:TetrisRT")
# TetrisRTKStep - same as TetrisRT but enables K-step action-delay MCTS in the agent.
register(id="TetrisRTKStep-v0", entry_point="jumanji.environments:TetrisRTKStep")
# TetrisRTKT - same as TetrisRT but uses policy-guided (not noop) delay steps in MCTS.
register(id="TetrisRTKT-v0", entry_point="jumanji.environments:TetrisRTKT")
...
register(id="Connector-v3", entry_point="jumanji.environments:Connector")
...
# SnakeKT - same as Snake but uses policy-guided (not no-op) delay steps in MCTS.
register(id="SnakeKT-v1", entry_point="jumanji.environments:SnakeKT")
...
# PacManKT - same as PacMan but uses policy-guided (not noop) delay steps in MCTS.
register(id="PacManKT-v1", entry_point="jumanji.environments:PacManKT")
