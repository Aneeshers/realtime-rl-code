...
class PacManKT(PacMan):
    """PacMan with policy-guided delay steps (KT = K-step with policy Transitions).

    Identical env dynamics to PacMan in every way.  The distinct class name
    enables isinstance dispatch in GumbelAlphaZeroAgent.

    During the K-1 delay steps, the agent uses argmax(masked_policy_logits)
    instead of noop (action=4, stay-in-place).  This means the MCTS tree
    simulates what the agent will actually do during the delay window, including
    respecting the valid-action mask at each step.

    Inherits _observation_from_state(state) from PacMan, which is called by
    _recurrent_fn to reconstruct the observation from the MCTS node state
