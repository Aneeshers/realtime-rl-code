...
class SnakeKT(Snake):
    """Snake with policy-guided delay steps (KT = K-step with policy Transitions).

    Identical env dynamics to Snake in every way.  The distinct class name
    enables isinstance dispatch in GumbelAlphaZeroAgent.

    During the K-1 delay steps, the agent uses argmax(masked_policy_logits)
    instead of a no-op, respecting the valid-action mask at each step.
    Both the MCTS tree expansion and the outer eval loop use these
    policy-guided K-step dynamics.

    Inherits _state_to_observation(state) from Snake, which is called by
    _recurrent_fn to reconstruct the observation at each MCTS node state
